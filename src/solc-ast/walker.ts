...
export class SolcAstWalker extends EventEmitter {
    ...
    walkInternal(ast: SolcAstNode,
     callbackPre: Function | undefined,
     callbackPost: Function | undefined
    ) {
      // console.log(`XXX id ${ast.id}, nodeType: ${ast.nodeType}, src: ${ast.src}`);
      // Here parent and children are available.
      if (callbackPre !== undefined) callbackPre(ast);
      ...
      // Here parent and children are available.
      if (callbackPost !== undefined) callbackPost(ast);
    }

  getChildren(astItem: SolcAstNode): Array<SolcAstNode> {
    const children: Array<SolcAstNode> = [];
  ...
  walk(ast: SolcAstNode, callbackPre: Function | undefined, callbackPost: Function| undefined) {
